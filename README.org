...
- Rich read mode:
  ...
  - Jump by section
  - Seamless page break looks like document in one piece.
...
Some RFCs are only present as PDF (e.g. RFC 8).  rfcview automatically falls back to PDF when a plain-text edition returns a 404 response, and also checks for a locally cached =.pdf= file before attempting a download.
