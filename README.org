...
- Browse the full IETF RFC index with RFC number in the left margin, author, date, and cross-reference links
...
- Rich read mode: section-heading highlighting, seamless page-break hiding (footer, form feed, running header, and padding lines are all suppressed so the document reads as continuous text), clickable =RFC XXXX= cross-references, and section navigation with =[= / =]=
...
To always open RFCs as PDF (even when a plain-text edition exists), set =rfcview:preferred-format=:

#+BEGIN_SRC emacs-lisp
(setq rfcview:preferred-format 'pdf)
#+END_SRC
